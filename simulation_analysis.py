...
for i in range(n+1):
    ...
    plt.imshow(np.flip(mean_ronch[i,:].reshape(120,140),0),cmap='inferno')
...
for i in range(n+1):
    ...
    plt.imshow(np.flip(std_ronch[i,:].reshape(120,140),0),cmap='inferno')
    plt.title('cluster_%s'%i)
    plt.axis('off')
    plt.axis('square')
    plt.axis('tight')
...
for i in range(n+1):
    ...
    plt.imshow(np.flip((1/L[i,:]).reshape(37,64),0),cmap=plt.cm.viridis)
    plt.title('cluster_%s'%i)
    plt.axis('off')
    plt.axis('square')
    plt.axis('tight')
