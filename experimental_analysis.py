...
for i in range(n+1):
    ...
    plt.imshow(mean_ronch_subtracted[i,:].reshape(180,180).T,cmap='inferno')
    plt.colorbar()
...
for i in range(n+1):
    ...
    plt.imshow(mean_ronch[i,:].reshape(180,180).T,cmap='inferno',vmin=8000,vmax=9800)
    plt.colorbar()
    plt.title('cluster_%s'%i)
    plt.axis('off')
    plt.axis('square')
...
for i in range(n+1):
    ...
    ax = fig.add_subplot(3,4,(i+1))
    plt.imshow(np.flip((1/L[i,:]).reshape(64,64),0),cmap='viridis')
    plt.title('cluster_%s'%i)
    plt.axis('off')
    plt.axis('square')
    plt.axis('tight')
